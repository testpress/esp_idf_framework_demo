...
CAM_IMAGE_MODE legacyMode(ArducamCamera* camera, CAM_IMAGE_MODE mode)
{
    if(camera->cameraId < SENSOR_5MP){ // legacy sensor
        switch(mode){
            ...
            default: break; // Handle other modes
        }
    }
    return mode;
}
                                  ...
                                  uint8_t num)
{
    ...
    // num is uint8_t, so it can't exceed CAPRURE_MAX_NUM (0xff)
...
void cameraWaitI2cIdle(ArducamCamera* camera)
{
    ...
    while ((readReg(camera, CAM_REG_SENSOR_STATE) & 0X03) != CAM_REG_SENSOR_STATE_IDLE) {
        if (++timeout > 10000) { // Timeout after ~10k iterations to prevent infinite loop
            printf("Warning: cameraWaitI2cIdle timeout - camera may not be connected\n");
            break;
        }
