...
static const char *AUTH_HEADER_VALUE =
    "Bearer iKnCz7E2Mtfl_V0bDcasJWDMzVN39L_BCySvj1hLDSc";
...
// ---- Base64 Printing Function ----
static void print_base64_image(const uint8_t *data, size_t len) {
  size_t out_len = 0;
  // Calculate required size for base64: (4 * n / 3) rounded up to nearest 4,
  // plus null terminator
  size_t buf_size = ((4 * len / 3) + 3) & ~3;
  ...
  int ret = mbedtls_base64_encode(base64_buf, buf_size, &out_len,
                                  (const unsigned char *)data, len);
  if (ret != 0) {
    ESP_LOGE("BASE64", "Base64 encoding failed with error %d", ret);
  ...
  printf("\r\n[IMAGE_BASE64_START]\r\n");
  // Print the entire string. For large images, this might be slow or hit buffer
  // limits, but typically serial monitor can handle it.
  printf("%s", (char *)base64_buf);
  printf("\r\n[IMAGE_BASE64_END]\r\n\r\n");
...
static const uint8_t rgb565_r_lut[32] = {
    ...
    0x58, 0x60, 0x68, 0x70, 0x78, 0x80, 0x88, 0x90, 0x98, 0xA0, 0xA8,
...
static const uint8_t rgb565_g_lut[64] = {
    ...
    0x84, 0x88, 0x8C, 0x90, 0x94, 0x98, 0x9C, 0xA0, 0xA4, 0xA8, 0xAC,
    0xB0, 0xB4, 0xB8, 0xBC, 0xC0, 0xC4, 0xC8, 0xCC, 0xD0, 0xD4, 0xD8,
    0xDC, 0xE0, 0xE4, 0xE8, 0xEC, 0xF0, 0xF4, 0xF8, 0xFC};
...
static inline void rgb565_to_rgb666_optimized(const uint16_t *src, uint8_t *dst,
                                              int count) {
  int i = 0;
  ...
  for (; i < unroll_count; i += 16) {
    ...
    uint16_t c8 = src[i + 8], c9 = src[i + 9], c10 = src[i + 10],
             c11 = src[i + 11];
    uint16_t c12 = src[i + 12], c13 = src[i + 13], c14 = src[i + 14],
             c15 = src[i + 15];

    uint8_t *d = dst + 3 * i;

    d[0] = rgb565_r_lut[(c0 >> 11) & 0x1F];
    d[1] = rgb565_g_lut[(c0 >> 5) & 0x3F];
    d[2] = rgb565_b_lut[c0 & 0x1F];
    d[3] = rgb565_r_lut[(c1 >> 11) & 0x1F];
    d[4] = rgb565_g_lut[(c1 >> 5) & 0x3F];
    d[5] = rgb565_b_lut[c1 & 0x1F];
    d[6] = rgb565_r_lut[(c2 >> 11) & 0x1F];
    d[7] = rgb565_g_lut[(c2 >> 5) & 0x3F];
    d[8] = rgb565_b_lut[c2 & 0x1F];
    d[9] = rgb565_r_lut[(c3 >> 11) & 0x1F];
    d[10] = rgb565_g_lut[(c3 >> 5) & 0x3F];
    d[11] = rgb565_b_lut[c3 & 0x1F];
    d[12] = rgb565_r_lut[(c4 >> 11) & 0x1F];
    d[13] = rgb565_g_lut[(c4 >> 5) & 0x3F];
    d[14] = rgb565_b_lut[c4 & 0x1F];
    d[15] = rgb565_r_lut[(c5 >> 11) & 0x1F];
    d[16] = rgb565_g_lut[(c5 >> 5) & 0x3F];
    d[17] = rgb565_b_lut[c5 & 0x1F];
    d[18] = rgb565_r_lut[(c6 >> 11) & 0x1F];
    d[19] = rgb565_g_lut[(c6 >> 5) & 0x3F];
    d[20] = rgb565_b_lut[c6 & 0x1F];
    d[21] = rgb565_r_lut[(c7 >> 11) & 0x1F];
    d[22] = rgb565_g_lut[(c7 >> 5) & 0x3F];
    d[23] = rgb565_b_lut[c7 & 0x1F];
    d[24] = rgb565_r_lut[(c8 >> 11) & 0x1F];
    d[25] = rgb565_g_lut[(c8 >> 5) & 0x3F];
    d[26] = rgb565_b_lut[c8 & 0x1F];
    d[27] = rgb565_r_lut[(c9 >> 11) & 0x1F];
    d[28] = rgb565_g_lut[(c9 >> 5) & 0x3F];
    d[29] = rgb565_b_lut[c9 & 0x1F];
    d[30] = rgb565_r_lut[(c10 >> 11) & 0x1F];
    d[31] = rgb565_g_lut[(c10 >> 5) & 0x3F];
    d[32] = rgb565_b_lut[c10 & 0x1F];
    d[33] = rgb565_r_lut[(c11 >> 11) & 0x1F];
    d[34] = rgb565_g_lut[(c11 >> 5) & 0x3F];
    d[35] = rgb565_b_lut[c11 & 0x1F];
    d[36] = rgb565_r_lut[(c12 >> 11) & 0x1F];
    d[37] = rgb565_g_lut[(c12 >> 5) & 0x3F];
    d[38] = rgb565_b_lut[c12 & 0x1F];
    d[39] = rgb565_r_lut[(c13 >> 11) & 0x1F];
    d[40] = rgb565_g_lut[(c13 >> 5) & 0x3F];
    d[41] = rgb565_b_lut[c13 & 0x1F];
    d[42] = rgb565_r_lut[(c14 >> 11) & 0x1F];
    d[43] = rgb565_g_lut[(c14 >> 5) & 0x3F];
    d[44] = rgb565_b_lut[c14 & 0x1F];
    d[45] = rgb565_r_lut[(c15 >> 11) & 0x1F];
    d[46] = rgb565_g_lut[(c15 >> 5) & 0x3F];
    d[47] = rgb565_b_lut[c15 & 0x1F];
  }
...
// LVGL flush callback
static void ili9488_flush(lv_display_t *disp, const lv_area_t *area,
                          uint8_t *px_map) {
  ...
  ESP_LOGI(TAG, "FLUSH: area=%dx%d, conv=%lldus, xfer=%lldus, total=%lldus", w,
           h, conv_end - conv_start, xfer_end - xfer_start,
...
static esp_err_t ft6206_init(uint8_t threshold) {
  ...
  // Write 0x00 to Monitor Mode Period Register (0x88) to keep chip in Active
  // Mode
...
static esp_err_t ft6206_read_touch(uint16_t *x, uint16_t *y,
                                   uint8_t *num_touches) {
  uint8_t data[16];
  ...
  *num_touches = data[FT6206_REG_NUMTOUCHES] & 0x0F;
  ...
  if (*num_touches > 0) {
    // Point 1 coordinates
    uint16_t raw_x =
        ((data[FT6206_REG_P1_XH] & 0x0F) << 8) | data[FT6206_REG_P1_XL];
    uint16_t raw_y =
        ((data[FT6206_REG_P1_YH] & 0x0F) << 8) | data[FT6206_REG_P1_YL];
...
#define TOUCH_Y_MAX 470
    ...
    int32_t mapped_x =
        ((int32_t)(TOUCH_Y_MAX - raw_y) * (HOR_RES - 1)) / TOUCH_Y_MAX;
    int32_t mapped_y = ((int32_t)(raw_x - TOUCH_X_MIN) * (VER_RES - 1)) /
...
static void hx711_do_tare(uint16_t samples) {
  ...
  ESP_LOGI(TAG, "[HX711 TARE] OFFSET set to %ld (avg of %u samples)",
           hx711_offset, samples);
}

static double hx711_get_weight_grams(void) {
  long raw =
      hx711_read_raw_average(10); // balanced averaging for stability and speed
...
static esp_err_t send_image_to_server(const uint8_t *jpeg, size_t jpeg_len,
  ...
  uint8_t *body =
      heap_caps_malloc(total_size + 1, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
  if (!body) {
    ESP_LOGE(TAG, "Failed to allocate PSRAM for multipart body size=%u",
             (unsigned)total_size);
  ...
  response.buffer =
      heap_caps_malloc(response.max_len, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
  response.len = 0;
  ...
  // Set POST body
  esp_err_t err =
      esp_http_client_set_post_field(client, (const char *)body, total_size);
  if (err != ESP_OK) {
  ...
  if (err == ESP_OK) {
    ...
    ESP_LOGI(TAG, "FastAPI Response (%d bytes): %s", response.len,
             response.buffer);
    ...
    if (status == 200 || status == 201) {
      ...
      if (tabview) {
        lv_tabview_set_active(tabview, 1,
                              LV_ANIM_OFF); // Instant switch, no animation
      }
      ...
      char status_msg[64];
      snprintf(status_msg, sizeof(status_msg), "Upload error - HTTP %d",
               status);
    ...
    case ESP_ERR_HTTP_CONNECT:
      ESP_LOGE(
          TAG,
          "Connection failed - check server IP/port and if server is running");
      break;
...
static esp_err_t capture_handler(httpd_req_t *req) {
  ...
  uint8_t *image_data =
      heap_caps_malloc(image_length, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
  if (!image_data) {
  ...
  if (bytes_read != image_length) {
    ESP_LOGE(TAG, "Incomplete read: %u/%u", (unsigned)bytes_read,
  ...
  esp_err_t post_err =
      send_image_to_server(image_data, image_length, current_weight);
  if (post_err != ESP_OK) {
  ...
  // Print base64 to monitor
  print_base64_image(image_data, image_length);
...
httpd_handle_t start_webserver(void) {
  ...
  if (httpd_start(&server, &config) == ESP_OK) {
    ...
    httpd_register_uri_handler(server,
                               &(httpd_uri_t){.uri = "/capture",
                                              .method = HTTP_GET,
                                              .handler = capture_handler,
                                              .user_ctx = NULL});
    httpd_register_uri_handler(server,
                               &(httpd_uri_t){.uri = "/favicon.ico",
                                              .method = HTTP_GET,
...
void stop_preview(void) { ESP_LOGI(TAG, "stop_preview() called"); }

/* -------------------------------------------------------------------
 * LVGL tick
 * -------------------------------------------------------------------*/
static void tick_inc(void *arg) { lv_tick_inc(1); }
...
static void back_to_capture_handler(lv_event_t *e) {
  ...
  if (code == LV_EVENT_CLICKED) {
    ...
    if (tabview) {
      lv_tabview_set_active(tabview, 0,
                            LV_ANIM_OFF); // Disable animation to reduce stutter
    }
  }
}
...
static void show_loading_screen(void) {
  if (!loading_screen) {
    ...
    lv_obj_set_style_bg_color(loading_screen, lv_color_hex(0x0A0E27),
                              LV_PART_MAIN);
    lv_obj_set_style_bg_opa(loading_screen, LV_OPA_80, LV_PART_MAIN);
    lv_obj_clear_flag(loading_screen, LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_clear_flag(loading_screen,
                      LV_OBJ_FLAG_CLICKABLE); // Block interactions
    ...
    lv_obj_set_style_arc_color(spinner, lv_color_hex(0x2196F3),
                               LV_PART_INDICATOR);
  }
...
static void update_results_display(void) {
  if (result_meal_name) {
    lv_label_set_text(result_meal_name,
                      meal_label[0] ? meal_label : "Unknown Food");
  }
...
static void create_camera_ui(void) {
  ...
  lv_obj_set_style_border_color(status_card, lv_color_hex(0x667EEA),
                                LV_PART_MAIN);
  ...
  lv_obj_set_style_border_color(weight_card, lv_color_hex(0x4ECDC4),
                                LV_PART_MAIN);
  ...
  lv_obj_set_style_border_color(results_card, lv_color_hex(0x667EEA),
                                LV_PART_MAIN);
  ...
  const char *nutrient_names[] = {
      "Weight:", "Calories:", "Protein:", "Carbs:", "Fat:"};
  lv_color_t nutrient_colors[] = {
      lv_color_hex(0x4ECDC4), lv_color_hex(0xFFE66D), lv_color_hex(0xFF6B6B),
      lv_color_hex(0xA78BFA), lv_color_hex(0xF093FB)};

  lv_obj_t **result_labels[] = {&result_weight, &result_calories,
                                &result_protein, &result_carbs, &result_fat};
  ...
  lv_obj_add_event_cb(back_btn, back_to_capture_handler, LV_EVENT_CLICKED,
                      NULL);
...
static void diagnostic_monitor_task(void *arg) {
  ESP_LOGI(TAG, "Diagnostic monitoring task started (priority %d)",
           TASK_PRIORITY_LOW);
...
static void camera_control_task(void *arg) {
  ESP_LOGI(TAG, "Camera control task started (priority %d)",
           TASK_PRIORITY_HIGH);
...
void app_main(void) {
  ...
  spi_bus_config_t buscfg = {.mosi_io_num = PIN_TFT_MOSI,
                             .miso_io_num = -1,
  ...
  /* LVGL tick */
  const esp_timer_create_args_t tick_args = {.callback = &tick_inc,
                                             .name = "lv_tick"};
  ...
  rgb666_buf =
      heap_caps_malloc(rgb666_buf_size, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
  if (!rgb666_buf) {
  ...
  size_t lvgl_buf_size_bytes =
      HOR_RES * VER_RES * sizeof(lv_color_t) + 8; // +8 bytes for palette
  lv_buf1 = heap_caps_malloc(lvgl_buf_size_bytes,
                             MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
  lv_buf2 = heap_caps_malloc(lvgl_buf_size_bytes,
                             MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
  ...
  lv_draw_buf_init(&draw_buf1, HOR_RES, VER_RES, LV_COLOR_FORMAT_RGB565,
                   LV_STRIDE_AUTO, lv_buf1, lvgl_buf_size_bytes);
  lv_draw_buf_init(&draw_buf2, HOR_RES, VER_RES, LV_COLOR_FORMAT_RGB565,
                   LV_STRIDE_AUTO, lv_buf2, lvgl_buf_size_bytes);
  ...
  lv_display_set_antialiasing(
      disp, false);              // Disable antialiasing for better performance
  ...
  BaseType_t r = xTaskCreatePinnedToCore(lvgl_task, "lvgl", LVGL_TASK_STACK,
                                         NULL, TASK_PRIORITY_UI, NULL, 1);
  if (r != pdPASS) {
  ...
  BaseType_t diag_r =
      xTaskCreate(diagnostic_monitor_task, "diag_mon", DIAGNOSTIC_TASK_STACK,
                  NULL, TASK_PRIORITY_LOW, NULL);
  if (diag_r != pdPASS) {
  ...
  BaseType_t cam_r =
      xTaskCreate(camera_control_task, "cam_ctrl", CAMERA_TASK_STACK, NULL,
  ...
  const uint32_t DIAGNOSTICS_TICKS =
      DIAGNOSTICS_INTERVAL_MS / portTICK_PERIOD_MS;
